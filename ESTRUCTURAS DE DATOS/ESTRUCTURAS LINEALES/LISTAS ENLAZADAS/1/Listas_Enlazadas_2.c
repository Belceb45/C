...
{
    // creando la lista
    NODE *start = NULL, *current; // crea puntero start con NULL para poner la lista vacia
    ...
    do
    {
        ...
        scanf("%d", &number);
        if (number)
        { // si el number es distinto de 0 continua
            if (!start)
            {                               // si start = NULL se crea el NODO (lista vacia)
            ...
            { // si lista no esta vacia,
                ...
                while (current->next)
                { // mientras current tenga algo en su next
